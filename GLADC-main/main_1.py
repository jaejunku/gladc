...
def train(dataset, data_test_loader, NetG, noise_NetG, args):    
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch+1)%10 == 0 and epoch > 0:
            ...
            optimal_idx = np.argmax(tpr_ab - fpr_ab)
            optimal_threshold = thresholds[optimal_idx]
            optimal_tpr = tpr_ab[optimal_idx]  # TPR at the optimal threshold
            ...
            # print(f'Number of graphs considered anomalous: {num_anomalous}')


            # print(f"Epoch {epoch+1}: Optimal TPR = {optimal_tpr}")
...
def oversample_graphs(graphs, labels):
    labels_array = np.array(labels)
    
    # Find indices of normal and anomalous graphs
    ...
    # Oversample anomaly graphs
    # Use sklearn's resample to duplicate anomalies
    oversampled_anomaly_indices = resample(anomaly_indices, replace=True, n_samples=len(normal_indices), random_state=123)
    
    # Combine the original normal indices with oversampled anomaly indices
    balanced_indices = np.concatenate([normal_indices, oversampled_anomaly_indices])
...
if __name__ == '__main__':
    ...
    for fold, (train_index, test_index) in enumerate(skf.split(all_graphs, labels)):
        ...
        # train_graphs, train_labels = oversample_graphs(train_graphs, train_labels)
            # Identify abnormal graphs
        # abnormal_graphs = [G for G in train_graphs if G.graph['label'] != 0]
        # Randomly select 25% of the abnormal graphs to remove
        # graphs_to_remove = random.sample(abnormal_graphs, int(len(abnormal_graphs) * 0.5))

        # Remove the selected abnormal graphs from the training set
        # train_graphs = [G for G in train_graphs if G not in abnormal_graphs]
        ...
        # Initialize models
        NetG = NetGe1(dataset_sampler_train.feat_dim, args.hidden_dim, args.output_dim, args.num_gc_layers, num_layers=2, bn=args.bn, args=args).cuda()
        ...
        noise_NetG = Encoder1(dataset_sampler_train.feat_dim, args.hidden_dim, args.output_dim, args.num_gc_layers,num_layers=2, bn=args.bn, args=args).cuda()
